...
import { salesReturnAPI, salesOrderAPI } from '../../services/api';
...
const SalesReturn = () => {
  ...
  const handleSubmit = async () => {
    try {
      if (selectedReturn) {
        await salesReturnAPI.update(selectedReturn._id, formData);
        toast.success('Sales return updated successfully');
      } else {
        await salesReturnAPI.create(formData);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Failed to delete sales return';
  ...
  const handleDownloadPDF = () => {
    try {
      ...
      // Prepare table data
      const tableColumn = [
        'Return ID', 'Order ID', 'Date', 'Customer', 'Product',
      ...
      // Save the PDF
      doc.save(`sales-returns-report-${new Date().toISOString().split('T')[0]}.pdf`);
    ...
    try {
      ...
      const excelData = filteredReturns.map(returnItem => {
        ...
        return {
          ...
          'Warehouse': returnItem.warehouse?.name || 'Main',
      ...
      // Save the Excel file
      XLSX.writeFile(wb, `sales-returns-report-${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ backgroundColor: '#fff3e0' }}>
                ...
                <TableCell sx={{ fontWeight: 600, color: '#ff9f43' }}>CUSTOMER</TableCell>
                        ...
                        <IconButton
                          onClick={() => {
                            ...
                            setFormData({
                              ...
                              customer: returnItem.customer?._id,
      ...
      <Dialog open={dialog} onClose={() => setDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ borderBottom: 1, borderColor: 'divider', color: '#ff9f43' }}>
          {selectedReturn ? 'Edit Sales Return' : 'Create Sales Return'}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <FormControl fullWidth>
              ...
              >
                {orders.map((order) => (
                  <MenuItem key={order._id} value={order.orderNumber}>
                    {order.orderNumber}
                  </MenuItem>
                ))}
              </Select>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={6}>
                  ...
                  <Typography variant="body1">{selectedReturn.customerName}</Typography>
                </Grid>
                <Grid item xs={6}>
